...
(function () {
  ...
  function showPage(page) {
    ...
    if (page === 'feedback') {
      ...
      $$('.fb-tab[data-sheet]').forEach((t, i) => t.classList.toggle('active', i === 0));
    }
    if (page === 'feedback-mat') renderFeedbackMaterials();
  ...
  function bindEvents() {
    ...
    // 课堂反馈 tab 切换
    $$('.fb-tab[data-sheet]').forEach(tab => {
      tab.onclick = () => {
        const sheet = tab.dataset.sheet;
        const sw = $('#fbSwipe');
        const target = sw && sw.querySelector('.fb-sheet[data-sheet="' + sheet + '"]');
        if (sw && target) sw.scrollTo({ left: target.offsetLeft, behavior: 'smooth' });
        $$('.fb-tab[data-sheet]').forEach(t => t.classList.toggle('active', t === tab));
      };
    });
    const fbSwipe = $('#fbSwipe');
    if (fbSwipe) {
      fbSwipe.addEventListener('scroll', () => {
        const idx = Math.round(fbSwipe.scrollLeft / Math.max(1, fbSwipe.clientWidth));
        $$('.fb-tab[data-sheet]').forEach((t, i) => t.classList.toggle('active', i === idx));
      }, { passive: true });
    }
    ...
    const genClassFbClassSel = $('#genClassFbClass');
    if (genClassFbClassSel) genClassFbClassSel.onchange = autoFillClassFbStudents;
...
${source}`;
  ...
  // v3.8: 课堂反馈预设格式——对照老师真实反馈排版（标题/科目/教师/课次/名单/内容自动编号/表现/作业/家长回复）
...
☺☺家长收到请回复“1”（您的回复是我们反馈的最大动力）[玫瑰][玫瑰]
@所有人`,
    concise:
`{标题}
科目：{科目}｜教师：{教师}｜课次：{课次}
...
家长收到请回复“1”@所有人`
  ...
  function numberLines(text) {
    ...
    return lines.map((l, i) => {
      if (/^[\d]+[、.．]/.test(l)) return l;        // 已编号则保留
      return (i + 1) + '、' + l;
    }).join('\n');
  }
  // 学生名单：折叠空格与换行，整理成一行姓名
  ...
  function generateClassFeedbackFromTemplate() {
    ...
    const title = ($('#genClassFbTitle') && $('#genClassFbTitle').value.trim()) || '语文课程反馈';
    const subject = ($('#genClassFbSubject') && $('#genClassFbSubject').value.trim()) || '';
    const teacher = ($('#genClassFbTeacher') && $('#genClassFbTeacher').value.trim()) || '';
    const lesson = ($('#genClassFbLesson') && $('#genClassFbLesson').value.trim()) || '';
    const students = normalizeNames(($('#genClassFbStudents') && $('#genClassFbStudents').value) || '');
    let content = numberLines($('#genClassFbContent').value);
    ...
    let result = tpl
      .replace(/\{标题\}/g, title)
      .replace(/\{科目\}/g, subject || '（待补充科目）')
      .replace(/\{教师\}/g, teacher || '（待补充教师）')
      .replace(/\{课次\}/g, lesson || '（待补充课次）')
      .replace(/\{名单\}/g, students || '（待补充学生名单）')
      .replace(/\{内容\}/g, content || '（待补充本节课内容）')
      .replace(/\{表现\}/g, perf || '（待补充学生表现）')
      .replace(/\{作业\}/g, hw || '（待补充家庭作业）')
      .replace(/\{素材\}/g, matText)
      .replace(/\{班级\}/g, cname).replace(/\{日期\}/g, date); // 兼容旧占位符
  ...
  // 选班级后自动带出该班学员名单（从学情台账同名班级）
  function autoFillClassFbStudents() {
    const cid = $('#genClassFbClass') && $('#genClassFbClass').value;
    const ta = $('#genClassFbStudents');
    if (!cid || !ta) return;
    const names = state.ledgerStudents.filter(s => s.classId === cid).map(s => (s.name || '').trim()).filter(Boolean);
  ...
  async function saveClassFeedbackFromTemplate() {
    ...
    const item = {
      id: uid(), classId: cid, date,
      title: ($('#genClassFbTitle') && $('#genClassFbTitle').value.trim()) || '',
      subject: ($('#genClassFbSubject') && $('#genClassFbSubject').value.trim()) || '',
      teacher: ($('#genClassFbTeacher') && $('#genClassFbTeacher').value.trim()) || '',
      lesson: ($('#genClassFbLesson') && $('#genClassFbLesson').value.trim()) || '',
      students: normalizeNames(($('#genClassFbStudents') && $('#genClassFbStudents').value) || ''),
      content, performance, homework, ts: Date.now()
    };
...
${note ? '补充背景：' + note : ''}`;
  ...
  function queryExpressByPhone() {
    ...
    if (!phone) { toast('请输入取件码'); return; }
    const el = $('#phoneExpressResult');
    const matched = state.express.filter(e => (e.no || '').includes(phone));
    let html = '';
    if (matched.length > 0) {
      html += `<p style="font-size:13px;font-weight:600;margin-bottom:8px">找到 ${matched.length} 条：</p>`;
      html += matched.map(e => {
        ...
        return `
          <div class="record-item" style="flex-direction:column;align-items:flex-start;gap:4px">
            <div style="display:flex;justify-content:space-between;width:100%;align-items:center">
              <span style="font-size:11px;color:var(--text-muted)">${isCode ? '取件码' : '单号'}｜${fmtDate(e.ts).slice(5,10)}</span>
            </div>
      ...
      html += '<p style="font-size:13px;color:var(--text-muted);padding:12px;text-align:center">未找到该取件码</p>';
      html += '<p style="font-size:12px;color:var(--text-soft);padding:0 12px">提示：在上方「取件码记录」里保存 号码 + 地点 后，就能在这里凭号码查到对应地点</p>';
    }
    html += `
  ...
  function renderClassFeedbackList() {
    ...
    el.innerHTML = list.map(f => {
      ...
      return `
        <div class="student-card" style="position:relative">
          <h4>${escapeHtml(f.title || cname)} <span class="student-tag">${escapeHtml(f.date||'')}</span></h4>
          ${f.subject || f.teacher || f.lesson ? `<p style="font-size:11px;color:var(--text-muted);margin-top:2px">${escapeHtml([f.subject, f.teacher, f.lesson].filter(Boolean).join(' ｜ '))}</p>` : ''}
  ...
  function editClassFeedbackModal(id) {
    ...
    openModal(existing ? '编辑课堂反馈' : '录入课堂反馈', `
      ...
      <label>课程标题<input type="text" id="cfTitle" style="font-size:16px" value="${escapeHtml(existing ? existing.title : '☀2026学霸益优语文课程反馈')}"></label>
      <label>科目<input type="text" id="cfSubject" style="font-size:16px" value="${escapeHtml(existing ? existing.subject : '')}" placeholder="【语文】七年级暑假拔创"></label>
      <label>教师<input type="text" id="cfTeacher" style="font-size:16px" value="${escapeHtml(existing ? existing.teacher : '')}" placeholder="何老师"></label>
      <label>课次<input type="text" id="cfLesson" style="font-size:16px" value="${escapeHtml(existing ? existing.lesson : '')}" placeholder="暑假第4次"></label>
      <label>学生名单<textarea id="cfStudents" rows="2" style="font-size:16px" placeholder="熊子骏 高朗绮 蔡诗瑶 ...">${escapeHtml(existing ? existing.students : '')}</textarea></label>
    ...
    $('#cfSave').onclick = async () => {
      ...
      const extra = {
        ...
        subject: ($('#cfSubject') && $('#cfSubject').value.trim()) || '',
        teacher: ($('#cfTeacher') && $('#cfTeacher').value.trim()) || '',
        lesson: ($('#cfLesson') && $('#cfLesson').value.trim()) || '',
        students: normalizeNames(($('#cfStudents') && $('#cfStudents').value) || '')
      };
      if (existing) {
        Object.assign(existing, { classId, date, content, performance, homework, ts: Date.now() }, extra);
        await dbPut('classFeedbacks', existing);
      } else {
        const item = { id: uid(), classId, date, content, performance, homework, ts: Date.now(), ...extra };
        state.classFeedbacks.push(item);
        await dbPut('classFeedbacks', item);
      }
      saveLocalCache();
    ...
    el.innerHTML = state.express.slice().reverse().map(e => {
      ...
      return `
        <li class="record-item" style="flex-direction:column;align-items:flex-start;gap:6px">
          <div style="display:flex;justify-content:space-between;width:100%;align-items:center">
            <span style="font-size:11px;color:var(--text-muted)">${label}｜${fmtDate(e.ts).slice(5,10)}</span>
          ...
          ${e.location ? `<div style="font-size:13px;color:var(--text-soft)">📍 ${escapeHtml(e.location)}</div>` : ''}
        </li>
      `;
  ...
  async function addExpress() {
    ...
    if (!no) { toast('请填写取件码 / 单号（号码）'); return; }
            ...
            <table class="score-table ledger-table">
              ...
              <tbody>
                ${students.map(s => {
                  ...
                  const sc = s.scores || [];
                  const isFullRec = r => r && r.fullScore != null && r.fullScore !== '' && Number(r.fullScore) !== 0 && Number(r.score) === Number(r.fullScore);
                  const lastEt = et[et.length - 1];
                  ...
                  <tr>
                    ...
                    <td class="${etFull ? 'full-mark' : ''}">${etText}</td>
                    <td class="${hwFull ? 'full-mark' : ''}">${hwText}</td>
  ...
  window.quickLedgerAdd = async function () {
    ...
    if (!sid) { toast('请先选择学员'); return; }
    const s = state.ledgerStudents.find(x => x.id === sid);
    if (!s) { toast('学员不存在，请重新选择'); renderLedgerQuick(); return; }
    normalizeLedger(s);
    const type = window.__lqType;
    const date = ($('#lqDate').value || '').trim() || todayStr();
    const note = ($('#lqNote').value || '').trim();
    const score = Number($('#lqScore').value || 0) || 0;
    const fullRaw = ($('#lqFull') ? $('#lqFull').value : '').trim();
    const fullScore = fullRaw !== '' ? Number(fullRaw) : null;
    const isFull = fullScore != null && fullScore !== 0 && Number(score) === fullScore;
    // 满分自动加 👍（避免重复叠加）
    let noteFinal = note;
